...
**This file is the single source of truth, and it is tool-agnostic.** Codex, Cursor, Zed, Aider and
Jules read it natively. Other tools get a thin generated pointer back to this file — those pointers
carry no instructions of their own, so nothing here needs duplicating anywhere else.

If you are an agent whose config file pointed you here: this is the whole of it. Do not write rules
into a tool-specific file; edit this one and run `sh .agents/scripts/sync-agents.sh`. See
[.agents/README.md](.agents/README.md) for the layout.
...
### Tool independence
- **Do not add tool-specific instruction files with content of their own.** Rules go in this file.
  Adapters are generated by `.agents/scripts/sync-agents.sh` and are pointers, never sources.
- Keep anything a build or run depends on in portable places — `package.json` scripts,
  `vite.config.ts` — so no tool is required to work on this repository.
- Tool-specific runtime config (editor settings, preview panes, launch files) is gitignored.
